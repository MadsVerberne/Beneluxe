...
class HuisjeController extends Controller
{
    ...
    // Formulier om nieuw huisje aan te maken
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('fotos')) {
            ...
            foreach ($order as $volgorde => $index) {
                ...
                $file = $files[$index];
                $path = $file->store('huisjes', 'public');
    ...
    // Formulier om een bestaand huisje te bewerken
    public function edit(Huisje $huisje)
    {
        $voorzieningen = Voorzieningen::all();
        return view('huisjes.edit', compact('huisje', 'voorzieningen'));
    }

    // Huisje bijwerken
    public function update(Request $request, Huisje $huisje)
    {
        $validated = $request->validate([
            'titel' => 'required|string|max:255',
            'beschrijving' => 'required|string',
        ...
        if ($request->hasFile('fotos')) {
            ...
            $maxVolgorde = $huisje->fotos()->max('volgorde') ?? 0;
        ...
        return redirect()->route('huisjes.index')->with('success', 'Huisje succesvol bijgewerkt!');
    }
}
